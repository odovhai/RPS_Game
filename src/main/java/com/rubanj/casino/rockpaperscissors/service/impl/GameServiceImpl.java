...
public class GameServiceImpl implements GameService {
    ...
    public RPSGame playRPS(RPSGameRequest rpsGameRequest) {
        ...
        UserGame userGame = userGameRepository.findById(rpsGameRequest.getGameId())
                .orElseThrow(() -> new NoSuchElementException(String.format("There is no game with id=[%s]", rpsGameRequest.getGameId())));

        int index = new Random().nextInt(RPSChoice.values().length);
        RPSChoice aiChoice = RPSChoice.values()[index];
